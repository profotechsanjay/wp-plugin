...
$UserID=$_COOKIE['mccuserid'];
$locationUserID=$_COOKIE['mccuserid'];

//$locationUserID=$_SESSION['general']['mcc_userid'];
...
<div class="row">
             <div class="col-md-12 col-xs-12 pad_bottom_10 text-center">
                                <div class="internal-div align-centre">
...
//	echo $time . " - " . time();
  ...
  $AnalyticsAccountData .= '<div class="main_account"><div class="account_label">Account Name</div><div class="account_name"><b>' . $CurClient[6] . '</b></div></div><div class="disconnect-button">';?>
       ...
       // $AnalyticsAccountData .= '<input class="btn btn-success btn-disconnect" type="button" value="Disconnect From Google Analytics"  OnClick="DoAction(\'' . $CurClient[0] . '\', \'DisconnectAnalytics\', \'\')" /></div>';
        ...
        $AnalyticsAccountData .= "<input class='btn btn-success btn-disconnect' data-id=$CurClient[0] type='button' value='Disconnect From Google Analytics'  OnClick='' /></div>";
                           ?>    
   ...
   $AnalyticsAccountData .= CreateHTMLSelectWithItems($AnalyticsChildAccountsList, $AnalyticsSelChildAccountID, 'SelectedAnalyticsChildAccount', $CurClient[0],'class="btn-select"','OnChange="DoAction(\'' . $CurClient[0] . '\', \'SelectedAnalyticsChildAccountChanged\', \'\')"');
 $AnalyticsAccountData .="</div>";
   $AnalyticsAccountData .="<div class='col-md-4 col-xs-12'>";
                            $AnalyticsAccountData .= '<label  class="child_a text-left mont-font">Select properties: </label>' . CreateHTMLSelectWithItems($AnalyticsWebPropertiesList, $AnalyticsSelWebPropertieID, 'SelectedAnalyticsWebPropertie', $CurClient[0],'class="btn-select1"', 'OnChange="DoAction(\'' . $CurClient[0] . '\', \'SelectedAnalyticsWebPropertieChanged\', \'\')"');
 $AnalyticsAccountData .="</div>";
   $AnalyticsAccountData .="<div class='col-md-4 col-xs-12'>";
                            $AnalyticsAccountData .= '<label  class="child_a text-left mont-font">Select profile: </label>' . CreateHTMLSelectWithItems($AnalyticsSelProfilesList, $AnalyticsSelProfileID, 'SelectedAnalyticsProfile', $CurClient[0],'class="btn-select2"');
 $AnalyticsAccountData .="</div>";
                    $AnalyticsAccountData .="</div>";
                    $AnalyticsAccountData .="</div>";
...
EOD;
  ...
  $AnalyticsAccountData .= "<input type='button' value='Connect Google Analytics' data-user='first' class='btn btn-success btn-connect connection' OnClick='DoGoogleAnalyticsRedirect()'  />";?>
                        ...
                        /*
                        $BTLAccountData .= '<i>
Select Bright Local Client: </i>' . CreateHTMLSelectWithItems($BTLChildAccountsList, $BTLChilActID, "SelectedBTLChildAccount", $CurClient[0], 'OnChange="DoAction(\'' . $CurClient[0] . '\', \'SelectedBTLChildAccountChanged\', \'\')"');
...
//if($FromCron)
 ...
 $ErrorMsg = "<pre>There is problem with your google Analytics account, User does not have any Google Analytics account.Please try Again </pre>";
             
                   $ErrorMsg .= " <input type='button' value='Connect Google Analytics' data-user='sec' class='btn btn-success btn-connect connection' OnClick='DoGoogleAnalyticsRedirect()'  />";
                ...
                <form name="AuthDemoForm" id="AuthDemoFormFormID" method="POST">
                    ...
                    <div  style="display: none;">
                        ...
                        <input type="hidden" name="ClientID" id="ClientIDEditID" />
                          <input type="hidden" name="locationid" id="locationid" value="<?php echo $CurClientID;?>" />
...
<div class="form-group m-b-0 bottom-footer radius-bottom">
        <div class="col-md-12">
            <a data-toggle="tab" id="tab-item3"  onclick="" data-id="<?php echo $locationUserID;?>" data-user="<?php echo user_id();?>"  data-website="<?php $url= get_user_meta($UserID, 'website', TRUE); echo appendhttp($url);?>" class="btn btn-success">Next <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></a>
        </div>
    </div>
 </div>
<style>
...
<div id="ga_connect_data" data-id="<?php echo $UserID; ?>"></div>
