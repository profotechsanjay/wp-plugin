...
<div class="contaninerinner trackdiv">     
    ...
    <div class="panel panel-primary">
        ...
        <div class="panel-body">
            ...
            <div class="gatrack">
                <?php
                if (isset($_REQUEST['location_id']) && intval($_REQUEST['location_id']) > 0) {
...
//                        pr($comp_total_keyword_limit);
//                        pr($total_comp_key);
//                        pr($keyword_opp_total_limit);
//                        pr($limit_check);
                        
                        if ((($comp_total_keyword_limit >= $total_comp_key) && ($keyword_opp_total_limit >= $package_kwo_used) && ($billing_enable == '1')) || ($billing_enable == '0')) {


                            if (isset($_POST["ownweb"]) && $_POST["ownweb"] != '') {
                                // own website run
                                echo '<div style="text-align:center;font-weight:bold;font-size:17px;color:green;">Report successfully run for own website. </div><div style="clear:both;height:20px;"></div>'; //To see your report instantly please <a href="'.$URI.'" target="_blank">click here</a>  
                                $url_name = $web_url;
                                $key_limit_for_url = isset($_POST["webkeylimit"]) ? intval($_POST["webkeylimit"]) : 250;
                                
                                // check if input greater than limit - then input will be our limit
                                if($billing_enable == '1'){
                                    $lt = $keyword_opp_total_limit - $package_kwo_used;
                                    if($lt < $key_limit_for_url){
                                        $key_limit_for_url = $lt;
                                    }
                                }

                                /*************20 Jan 2017*************/
                                //$country_code_btl = isset($_POST['btl_country_code'])?$_POST['btl_country_code']:"us";
                                ...
                                if (!isset($_POST['remove_0']) && !isset($_POST['remove_1']) && !isset($_POST['remove_2']) && !isset($_POST['remove_3'])) {
                                    ...
                                    } elseif (($billing_enable == '1') && ($max_keyword_get_limit < $key_limit_for_url)) {
                            ...
                            echo '<div class="keyword_alert">Competitor URL is not saved and Run, Because your Limit for <strong>Keyword Opportunity</strong> is over, So please contact to Administrator or Purchase <strong>Keyword Opp Add-Ons</strong> for increase Limit.</div>';
                        }
                    }
                    ?>
                    <div style="float:right; width:100%;">
                        ...
                        <form name="competitor_url_save_Frm" class="competitor_url_save_Frm" method="post" action="">
                            ...
                            <table>
                                ...
                                $competitor_url = get_user_meta($UserID, "competitor_url", true);
                                $web_url = get_user_meta($UserID, "website", true);
                                $last_run_date = $wpdb->get_var("SELECT `update_date` FROM `keyword_opportunity` WHERE `user_id` = '" . $UserID . "' && `competitor_info` like '%" . $web_url . "%' ORDER BY `update_date` DESC LIMIT 1");
                                if ($last_run_date == '') {
                                    $last_run_date = "Not Run";
                                }
                                $get_opp_url = $wpdb->get_var("SELECT COUNT(*) FROM `keyword_opportunity` WHERE `user_id` = '" . $UserID . "' && `competitor_info` like '%" . $web_url . "%'");

/*20 Jan Code*/
if(!empty(get_option("web_url_".$UserID))){ if(!empty($web_url) && $get_opp_url==0){ $get_opp_url="no keywords found"; } }else{  } /*Code Ends*/

                                $total_get_keyword_opp = $total_get_keyword_opp + $get_opp_url;
                                ?>
                                <tr>

                                    <td style="float:left; width:20%;"><b>Own URL :</b><br>Last Run : <?php echo $last_run_date; ?></td>

                                    <td style="float:left; width:38%;">
                                        <input style="width:100%;" type="url" name="weburl" data-id="<?php echo $web_url; ?>" readonly disabled value="<?php if (!empty($web_url)) echo $web_url; ?>" style="margin-bottom:4px">
                                    </td>
        <td style="float:left; width:12%;text-align:center;">
                                        <h1><strong class="count_<?php echo $web_url; ?>" style="font-size:13px;"><?php echo $get_opp_url; ?></strong></h1>                                           

                                    </td>
                                    <td style="float:left; width:10%;text-align: center;">                                           
                                        <input style="width:100%;" type="text" name="webkeylimit" value="250" style="margin-bottom:4px">
                                    </td>
                                    <td style="float:left; width:20%;">
                                        <input type="submit" style="padding: 8px 12px;" name="ownweb" value="Run/Update">                                            
                                    </td>
...
if(!empty($competitor_url[$com_url]) && $get_opp_url==0){ $get_opp_url="no keywords found"; }
                                    ...
                                    <tr>
                                        ...
                                        <td style="float:left; width:38%;">
                                            <input style="width:100%;" type="url" class="check_url_type" data-id="<?php echo $com_url; ?>" url="true" title="Please enter a complete URL to include the http or https" name="competitor_url[]" value="<?php if (!empty($competitor_url)) echo $competitor_url[$com_url]; ?>" style="margin-bottom:4px"><br/><span class="error_shw_message_<?php echo $com_url; ?>" style="color:red;"></span>
                                        </td>
                                        <td style="float:left; width:12%;text-align:center;">
                                            <h1><strong class="count_<?php echo $com_url; ?>" style="font-size:13px;"><?php echo $get_opp_url; ?></strong></h1>                                           
...
<script>
    jQuery(document).ready(function () {
	jQuery(".check_url_type").on("blur",function(){
		       var data_id = jQuery(this).attr("data-id");
		       if(jQuery(this).val()!=""){
	if(isUrlValid(jQuery(this).val())){ jQuery(".error_shw_message_"+data_id).html(""); }else{ jQuery(".error_shw_message_"+data_id).html("Please enter a complete URL to include the http or https"); return false;  }
	} else{ jQuery(".error_shw_message_"+data_id).html(""); }
