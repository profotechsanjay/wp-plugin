...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['stop_addons'])){
    ...
    if($resultsnew == 'success'){
        if($field_name == "location"){
            ...
            //array_merge($location_limits,$curent_delete_location);
            $deleted_location_serialize = serialize(array_merge($location_deleted,$curent_delete_location));
            $wpdb->query("UPDATE `wp_location_package_fields` SET `lpf_addons_delete` = '".$deleted_location_serialize."' WHERE `lpf_field` = '".$field_name."'");
...
function submitResult() {
    var check_confirm = confirm("Are you sure to Stop Add-Ons and Delete Location?");
    //alert(check_confirm);
    if(check_confirm){
        return true;
    }else{
