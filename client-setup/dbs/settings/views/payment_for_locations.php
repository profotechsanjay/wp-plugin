...
if(isset($_SESSION["packagetype"]) && isset($_SESSION["packageid"])){
    ...
    if($packagetype == 'discount_coupon'){
        ...
        $getpackage->dc_location_price = $discount_coupon['codedata']->dc_location_cost;
...
$lp_location_price = $getpackage->dc_location_cost;
$lp_locations = $getpackage->dc_location;
$ldc_location_cost = $getpackage->dc_location_cost;
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['location_payment_submit'])) {
    ...
    if(($_SESSION["packagetype"] == 'discount_coupon') && isset($_SESSION["packageprice"]) && ($_SESSION["packageprice"] == 0)){
        ...
        $xml = new AuthnetXML(AUTHNET_LOGIN, AUTHNET_TRANSKEY, 0);  // For Live Authorize Payment
        //$xml = new AuthnetXML(AUTHNET_LOGIN, AUTHNET_TRANSKEY, 1);  // For Sandbox Authorize Payment
    ...
    if($resultCode == "Ok" && $message_code == "I00001" && $message_success == "yes" && $message_error == "no"){
        ...
        if(!empty($billing_cancelon)){
         ...
         //code starts by rudra 20-jan-2017
                $db_name=$wpdb->dbname;
		$dbname=base64_encode($db_name);
		$dbname = isset($dbname)?$dbname:"";
		$url="http://admin.enfusen.com/billing-cron-status/?db_name=$dbname";
		$curl = curl_init();
		curl_setopt_array($curl, array(
		    CURLOPT_RETURNTRANSFER => 1,
		    CURLOPT_URL => $url
		));
		$results_me = curl_exec($curl);
		curl_close($curl);
	  //code ended by rudra 20-jan-2017 

        /******************** For Testing Porpose Only START *************************/
        $mail_to = 'roger@enfusen.com';
        $messages = '<pre>';
        $messages .= '<strong>Hi Roger,</strong>';
        $messages .= '<br><br>';
        $messages .= 'New Subscription of amount $.'.$amount.'. Start by '.$_POST['billToCompany'].'.';
        $messages .= '<br><br>';
        $messages .= '<strong>Regards</strong>';
        $messages .= '<br>';
        $messages .= '<strong>Enfusen</strong>';
        $messages .= '</pre>';
        ...
        mail($mail_to, 'Subscription Done', $messages, $headers);

        /******************** For Testing Porpose Only STOP *************************/
...
<form class="bill_form" method="post" action="">	
    ...
    <div class="col-md-5">
        <div class="bill_sec plan_div">
            ...
            <ul>
                <li><?php echo $lp_locations;?> Locations (each aditional $<?php echo $ldc_location_cost;?>pm)</li>
