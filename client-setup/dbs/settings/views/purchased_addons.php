...
if(!empty($deleted_locations) || $deleted_locations != 0){
...
foreach($get_addons_for_location as $get_addons_fields){
    //pr($get_addons_fields);
    $field = $get_addons_fields->addons_slug;
    
    if($field == 'keywords'){
        $key_field_price = $get_addons_fields->addons_cost;
        $key_field_limit = $get_addons_fields->addons_value;

    }elseif($field == 'comp_keywords'){
        $ckey_field_price = $get_addons_fields->addons_cost;
        $ckey_field_limit = $get_addons_fields->addons_value;

    }elseif($field == 'keyword_opp'){
        $keyo_field_price = $get_addons_fields->addons_cost;
        $keyo_field_limit = $get_addons_fields->addons_value;

    }elseif($field == 'pages'){
        $pages_field_price = $get_addons_fields->addons_cost;
        $pages_field_limit = $get_addons_fields->addons_value;

    }elseif($field == 'site_audit'){
        $audit_field_price = $get_addons_fields->addons_cost;
        $audit_field_limit = $get_addons_fields->addons_value;

    }elseif($field == 'citation_run'){
        $citation_field_price = $get_addons_fields->addons_cost;
        $citation_field_limit = $get_addons_fields->addons_value;

    }elseif($field == 'location'){
        
        $location_price = used_discountcode_inagency();
        $location_field_limit = 1;

    }elseif($field == 'demo_accounts'){
        $demo_accounts_price = $get_addons_fields->addons_cost;
        $demo_accounts_field_limit = $get_addons_fields->addons_value;
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['stop_addons'])){
    ...
    if($field_name == "keywords"){
        ...
        $amount = $demo_accounts_price;
        $limit = $demo_accounts_field_limit;
    }
