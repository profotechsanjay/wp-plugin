    ...
    if (isset($_REQUEST["param"])) {
        if ($_REQUEST["param"] == "add_setup") {                                  
            ...
            (
                $wpdb->prepare
                (
                    "SELECT count(id) FROM ". setup_table() . " WHERE prefix = %s ",
                    $prefix
                )
            );
            ...
            $hasrec = $wpdb->get_var
               (
                       $wpdb->prepare
                       (
                               "SELECT count(id) FROM ". setup_table() . " WHERE email = %s ",
                               $email
                       )
               );
            ...
            $hasrec = $wpdb->get_var
               (
                       $wpdb->prepare
                       (
                               "SELECT count(id) FROM ". setup_table() . " WHERE login = %s ",
                               $login
                       )
               );
            ...
            (
                ...
                (
                        ...
                        $name, $email, $login, $password, $prefix, $url, $dir, $user_id, $now, $user_id
                )
            );
            ...
            if($setup->client_id == ''){
                ...
                (
                    $wpdb->prepare
                    (
                            "UPDATE ". setup_table() . " SET client_id = %d WHERE id = %d",
                            $userid, $setup_id
                    )
                );
            }            
            ...
            if($key == ST_KEY){
                ...
                if(!empty($setup)){
                    ...
                    if(ST_ENABLE_CRONS == 1){
                        ...
                        /**************************** Code to add cron jobs for agency *******************************/
           ...
           if($key == ST_KEY){
                ...
                if(!empty($setup)){
                    ...
                    (
                        $wpdb->prepare
                        (
                                "GRANT ALL PRIVILEGES ON ".$analytic_db.".* TO '$user'@'$dbhost'",""
                        )
                    );
                    ...
                    (
                        $wpdb->prepare
                        (
                                "GRANT ALL PRIVILEGES ON ".$grader_db.".* TO '$user'@'$dbhost'",""
                        )
                    );
            ...
            $wpdb->query
                    ...
                    $wpdb->prepare
                            (
                            "UPDATE " . setup_table() . " SET status = %d WHERE id = %d",
                            $status, $setup_id
                    )
            );            
            ...
            (
                    $wpdb->prepare
                    (
                            "DELETE FROM ". setup_table() . " WHERE id = %d",
                            $setup_id
                    )
            );
...
//    );
    ...
    $igonre_tables = array("wp_setup_table","wp_client_company_info");  
    $igonre_folders = array(ST_BASEDIR."/wp-content/plugins/client-setup",
        ST_BASEDIR."/wp-content/plugins/settings", ST_BASEDIR."/.git",
    ...
    $igonre_pages = array('mashed-rss-feed','buyer-persona','task-manager','task-overview','site-audit-url-report','site-audit-report',
        ...
        'to-do-list','billing','email-subscription','not-assigned-task','task-manager-pdf','sales-report-new');
    ...
    $ignoretbls = array("wp_client_company_info","wp_setup_table","wp_location_mapping","wp_client_location","wp_citation_setup_instruction","wp_site_audit_instruction");
    ...
    if($setup_update == 0){
        ...
        (
            ...
            (
                "UPDATE ". setup_table() . " SET db_name = %s, analytic_db = %s, grader_db = %s, db_username = %s,"
                    . " db_password = %s, analytic_url = %s, analytic_dir = %s WHERE id = %d",
                $client_db, $analytic_db, $grader_db, $user, $password, $anlytics_url, $anlytics_dir, $id
            )
        );
        ...
        (
            $wpdb->prepare
            (
                "CREATE USER ".$user."@".$dbhost." IDENTIFIED BY '".$pass."'",""
            )
        );
        ...
        (
            ...
            (
                "UPDATE ". setup_table() . " SET white_lbl = '', db_name = %s, analytic_db = %s, grader_db = %s, "
                    . "analytic_url = %s, analytic_dir = %s WHERE id = %d",
                $client_db, $analytic_db, $grader_db, $anlytics_url, $anlytics_dir, $id
            )
        );
    ...
    (
        $wpdb->prepare
        (
                "GRANT ALL PRIVILEGES ON ".$client_db.".* TO '$user'@'$dbhost'",""
        )
    );          
//    if($per == FALSE){
