...
<!--                    <div class="alert alert-info">
                    ...
                    <div class="cronlist">
                      ...
                      if ($setup->db_name != ''){
                            ...
                            //$select_crons = $wpdb->get_results("Select cron_link,cron_time From `client_crons` where `agency_id`=$setup->id");
                            $loop_select_crons=$select_crons;
                            $setup_crons = count($select_crons);
                            if($setup_crons==0){
                              //$cronlist = ABSPATH . '/cron_list.php';
                              //$data = file_get_contents($cronlist); //old code 
                              $currentdate = date("Y-m-d h:i:s");
                              ...
                              $datas = $wpdb->get_results($wpdb->prepare("SELECT default_cron,cron_time,cron_type FROM master_crons where status=1",""));   //master "cron time" table
                                ...
                                foreach ($datajob as $key_val => $job) {
                                    ...
                                    $insert_crons = $wpdb->query(
                                            $wpdb->prepare(
                                                    "INSERT INTO enfusen_mcc_new.client_crons(agency_id, cron_link, cron_type, order_by, created) "
                                                    . "VALUES(%d, %s, %d, %d, NOW())", $setup->id, $job->default_cron, $job->cron_type, $setup->id
                                                    )                                      
                                      );

                                    $copidejobs .= $job->cron_time."  ".$job->default_cron."\n";
                                }
                            }else{
